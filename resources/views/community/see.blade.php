    ...
    <div class="flex flex-col md:flex-row md:space-x-6 p-6">
        ...
        <div class="flex-1">
            <div class="bg-white shadow-md rounded-lg p-4">
                <h1 class="text-4xl font-bold mb-4 text-gray-800">{{ $community->name }}</h1>

                <div class="mb-4">
                    <img src="{{ asset($community->image) }}" class="w-full h-32 object-cover rounded-lg mb-4" alt="{{ $community->name }}" />
                </div>

                <p class="text-gray-700 mb-4 text-lg">{{ $community->description }}</p>

                <a href="{{ route('community') }}" class="bg-blue-500 text-white px-6 py-3 rounded-lg hover:bg-blue-600 transition duration-200">
                    Back to Communities
                </a>
            </div>
        </div>
