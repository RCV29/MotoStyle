<x-app-layout>
    <h1 class="text-lg font-semibold text-center">Concern Details</h1>
    
    <div class="max-w-md mx-auto mt-6 p-4 bg-white rounded-lg shadow-md border border-gray-200">
        <div class="flex justify-center mb-4">
            <img src="{{ asset($community->image) }}" class="w-80 h-48 object-cover rounded-lg" alt="{{ $community->name }}" />
        </div>
        <h2 class="text-xl font-bold text-center mb-2">{{ $community->name }}</h2>
        <p class="text-gray-700 text-center">{{ $community->description }}</p>
    </div>
